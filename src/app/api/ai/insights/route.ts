...
export async function GET(request: NextRequest) {
  try {
    ...
    // Group items by meal
    const itemsByMeal = new Map<string, typeof mealItems>();
    ...
    for (const meal of recentMeals) {
      ...
      for (const item of items) {
        ...
        totalCalories += (snapshotData.nutrients?.energy_kcal || 0) * multiplier;
        totalProtein += (snapshotData.nutrients?.protein_g || 0) * multiplier;
      }
    }
